...
company_name = "한화"
file = pd.read_csv(f"./filtered_test/{company_name}_s.csv")
...
model = ESG_CNN()
# 학습된 가중치 불러오기
model.load_state_dict(torch.load(S_weight, map_location=torch.device("cpu"), weights_only=True))
model.eval()
# 모델을 평가 모드로 전환
