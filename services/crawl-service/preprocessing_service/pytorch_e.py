...
file_pattern = "*_e.csv"  # 예: 파일 이름에 '.csv'가 포함된 경우
...
def main():
  ...
  for file in csv_files:
    ...
    ESG_grade_2022 = matching_rows_2022["환경"].map(lambda grade: next((value for d in ESG_grade for key, value in d.items() if key == grade), 0))
    ESG_grade_2023 = matching_rows_2023["환경"].map(lambda grade: next((value for d in ESG_grade for key, value in d.items() if key == grade), 0))
    ESG_grade_2024 = matching_rows_2024["환경"].map(lambda grade: next((value for d in ESG_grade for key, value in d.items() if key == grade), 0))
...
def train_model(model, x_tensor, y_tensor, epochs=200, batch_size=32):
    ...
    torch.save(model.state_dict(), 'E_predictor.pth')
